...
module ChefCLI
  module Command
    ...
    class CleanPolicyRevisions < Base
      ...
      banner(<<~BANNER)
        Usage: #{ChefCLI::Dist::EXEC} clean-policy-revisions [options]

        `#{ChefCLI::Dist::EXEC} clean-policy-revisions` deletes orphaned Policyfile revisions from the
        #{ChefCLI::Dist::SERVER_PRODUCT}. Orphaned Policyfile revisions are not associated to any group, and
        therefore not in active use by any nodes. To list orphaned Policyfile revisions before deleting
        them, use `#{ChefCLI::Dist::EXEC} show-policy --orphans`.
